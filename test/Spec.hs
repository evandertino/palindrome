...
prop_preProcessInvariant text = preProcess text == cleanText
  where cleanText = (T.filter (not . isSpace) . T.filter (not . isPunctuation) . T.toLower) text

prop_isPalindromeInvariant text = isPalindrome text == isAPalindrome
  where 
    cleanPalindrome = T.filter (not . isSpace) . T.filter (not . isPunctuation) . T.toLower
